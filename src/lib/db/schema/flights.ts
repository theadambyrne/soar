...
export const insertFlightParams = baseSchema.extend({
...
export const updateFlightParams = baseSchema.extend({
  recordedAt: z.coerce.date()
}).omit({ 
  userId: true
});
...
export type CompleteFlight = Awaited<ReturnType<typeof getFlights>>["flights"][number];
